...
RUN set -eux; \
    export PHP_FPM_INSTALL_EXTENSIONS=${PHP_FPM_INSTALL_EXTENSIONS:-bcmath bz2 gd intl pdo_mysql zip}; \
    if [ "$DEVELOPMENT_MODE" == "1" ]; then \
...
RUN set -eux; \
    export PHP_CLI_INSTALL_EXTENSIONS=${PHP_CLI_INSTALL_EXTENSIONS:-bcmath bz2 gd intl pdo_mysql zip pcntl}; \
    if [ "$DEVELOPMENT_MODE" == "1" ]; then \
